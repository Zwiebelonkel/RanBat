...
function rollStat(rng, rarity, bonus) {
    const ranges = {
      common: { min: 0, max: 10 },
      rare: { min: 5, max: 20 },
      epic: { min: 15, max: 35 },
...
function rerollStat(character, stat) {
    const rng = makeRng(Date.now());
    const newStatValue = rollStat(rng, character.rarity, 0);
...
function generate(seed) {
    ...
    const { rarity, spread } = pickWeighted(
      rng,
      rarityWeights
    );
    const { tier, statBonus } = pickWeighted(rng, tierWeights);
    ...
    const power = hasPower
      ? pick(
          rng,
          powers.filter((p) => p !== 'Keine')
        )
      : undefined;

      const stats = {
        strength: rollStat(rng, rarity, statBonus),
        speed: rollStat(rng, rarity, statBonus),
        stamina: rollStat(rng, rarity, statBonus),
        defense: rollStat(rng, rarity, statBonus),
      };
...
function makeName(rng) {
    const firstName = pick(rng, humanNames);
    const lastName = pick(rng, humanSurnames);
